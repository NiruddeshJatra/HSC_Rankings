...
urlpatterns = [
    ...
    # Current URL scheme (year restricted to 4 digits so it doesn't shadow legacy redirects below)
    re_path(r'^(?P<exam>hsc|ssc)/(?P<year>\d{4})/individual/(?P<roll_no>[^/]+)/$', views.individual_result, name='individual'),
    re_path(r'^(?P<exam>hsc|ssc)/(?P<year>\d{4})/(?P<group>[^/]+)/$', views.results, name='results'),
